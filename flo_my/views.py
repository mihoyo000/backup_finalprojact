...
import fitz, json, openai
from weasyprint import HTML

# flo_my 앱의 모델과 폼을 import 합니다.
from .models import UploadedPDF, TestSet, Question, Choice, UserTestAttempt, UserAnswer, LearningGoal
from .forms import PDFUploadForm, LearningGoalForm, TestSetSearchForm, IncorrectNoteSearchForm
...
def pdf_upload_view(request):
    if request.method == 'POST':
        ...
        form = PDFUploadForm()
    return render(request, 'flo_my/pdf_upload.html', {'form': form}) # 템플릿 경로 수정
...
def select_num_questions_view(request, pdf_pk):
    ...
    return render(request, 'flo_my/select_num_questions.html', context) # 템플릿 경로 수정
...
def take_test_view(request, test_set_pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('flo_my:test_result', attempt_pk=attempt.pk)
    
    context = {
        'test_set': test_set,
        'questions': test_set.questions.all().prefetch_related('choices'),
    }
    return render(request, 'flo_my/take_test.html', context)
...
def test_result_view(request, attempt_pk):
    ...
    return render(request, 'flo_my/test_result.html', context)
...
def retake_incorrect_answers_view(request, attempt_pk):
    ...
    return render(request, 'flo_my/retake_incorrect_test.html', context)
...
def retake_result_view(request):
    ...
    return render(request, 'flo_my/retake_result.html', context)
